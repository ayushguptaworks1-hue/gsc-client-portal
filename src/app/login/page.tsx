...
export default function LoginPage() {
  ...
  const router = useRouter();
    ...
    useEffect(() => {
      const session = localStorage.getItem('gsc_client_session');
      if (session === 'true') {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
          // store full session for iframe/local fallback
          const sessionData = {
            authenticated: true,
            companyName: data.companyName,
            clientId: data.clientId,
            hiredMembers: data.hiredMembers || []
          };
          try {
            localStorage.setItem('gsc_client_session', 'true');
            localStorage.setItem('gsc_client_session_data', JSON.stringify(sessionData));
          } catch (e) {
    ...
    const sendHeight = () => {
      try {
        if (window.parent && window.parent !== window) {
          // Send smaller height for login page so form appears centered
          window.parent.postMessage({ type: 'iframeHeight', height: 700 }, '*');
        }
      } catch (e) {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center justify-center relative overflow-x-hidden py-12">
      ...
      <div className="relative z-10 w-full max-w-[420px] px-6">
        ...
        {/* Card */}
        <div id="login-card" className="bg-white rounded-2xl border border-slate-200 p-7 shadow-xl shadow-slate-200/50">
          {/* Error */}
          {error && (
            ...
            <div>
              ...
              <div className={`relative rounded-xl transition-all duration-200 ${focusedField === 'password' ? 'ring-2 ring-blue-500/30' : ''}`}>
                ...
                <input
                  ...
                  required
                />
                {/* Only one eye icon for show/hide password */}
                <button
                  type="button"
